...
import { loginByPassword, register } from '@/api/user'
...
async function handleSubmit() {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      if (isRegister.value) {
        // 注册
        result = await register(form.username, form.password, form.phone || undefined)
        ElMessage.success('注册成功')
      } else {
        // 登录
        result = await loginByPassword(form.username, form.password)
        ElMessage.success('登录成功')
      }

      console.log('登录/注册返回数据：', result)

      // 检查返回数据结构
      if (!result || !result.token) {
        ElMessage.error('响应数据格式错误')
        console.error('期望数据格式：{ token, refreshToken, user }，实际返回：', result)
        return
      }
